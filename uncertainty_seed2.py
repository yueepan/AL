...
ROOT_DIR = "./VisDrone"
OUTPUT_DIR = "./VisDrone/uncertainty_seeds"
...
tracker = eco2ai.Tracker(
    project_name="Uncertainty Seed2 Carbon", 
    experiment_description="training <uncertainty seed2> model",
...
initial_seed_path = os.path.join(OUTPUT_DIR, "initial_seed")
initial_train_images_folder = os.path.join(initial_seed_path, 'train', 'images')
initial_val_images_folder = os.path.join(initial_seed_path, 'val', 'images')
initial_train_labels_folder = os.path.join(initial_seed_path, 'train', 'labels')
initial_val_labels_folder = os.path.join(initial_seed_path, 'val', 'labels')


initial_train_images = os.listdir(initial_train_images_folder)
initial_val_images = os.listdir(initial_val_images_folder)

print(f"Initial Seed: Train images - {len(initial_train_images)}, Val images - {len(initial_val_images)}")

cumulative_train_images = set(initial_train_images)
cumulative_val_images = set(initial_val_images)
...
# Load YOLO model
model_path = os.path.join(OUTPUT_DIR, "initial_seed", 'best_initial_seed.pt')
...
discrepancy = 55 #change the number if images with the same score can't be selected accordingly 
...
if not os.path.exists(seed_folder):
    ...
    # Copy images and labels from the initial seed
    for file in initial_train_images:
        shutil.copy(os.path.join(initial_train_images_folder, file), new_train_images_folder)
        shutil.copy(os.path.join(initial_train_labels_folder, file.replace('.jpg', '.txt')), new_train_labels_folder)

    for file in initial_val_images:
        shutil.copy(os.path.join(initial_val_images_folder, file), new_val_images_folder)
        shutil.copy(os.path.join(initial_val_labels_folder, file.replace('.jpg', '.txt')), new_val_labels_folder)
    
    print(f"After copying from the initial seed: Train images - {count_images_in_folder(new_train_images_folder)}, Val images - {count_images_in_folder(new_val_images_folder)}")
    ...
    print(f"Final Seed 2: Train images - {final_train_count_seed2}, Val images - {final_val_count_seed2}")

    # Check if the number of images meets the expected count
    expected_train_count = 1042
    # expected_train_count = len(initial_train_images) + len(additional_images_train)
    assert final_train_count_seed2 == expected_train_count, f"Discrepancy in train images count. Expected: {expected_train_count}, Found: {final_train_count_seed2}"
 
assert final_train_count_seed2 == 1042, "Incorrect number of training images in seed2"

# Create YAML file for seed2
yaml_dict = {
            "path": ROOT_DIR,
            "train": os.path.join("uncertainty_seeds/", seed_name, "train/images"),
            "val": os.path.join("uncertainty_seeds/", seed_name, "val/images"),
            "names": {
                0: "pedestrian",
                1: "people",
                2: "bicycle",
