...
def load_sodaa(img_dir, ann_dir=None, nproc=10):
    ...
    if nproc > 1:
        pool = Pool(nproc)
        contents = pool.map(_load_func, sorted(os.listdir(img_dir)))
        pool.close()
    else:
        contents = list(map(_load_func, sorted(os.listdir(img_dir))))
...
def _load_sodaa_single(imgfile, img_dir, ann_dir):
