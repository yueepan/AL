...
diversity_scores_file = os.path.join(ROOT_DIR, 'diversity_seeds', 'diversity_scores.npz')
...
tracker = eco2ai.Tracker(
    ...
    experiment_description="hybrid seed2 model",
...
seed1_path = os.path.join(OUTPUT_DIR, "seed1")
seed1_train_images_folder = os.path.join(seed1_path, 'train', 'images')
seed1_val_images_folder = os.path.join(seed1_path, 'val', 'images')
seed1_train_labels_folder = os.path.join(seed1_path, 'train', 'labels')
seed1_val_labels_folder = os.path.join(seed1_path, 'val', 'labels')


seed1_train_images = os.listdir(seed1_train_images_folder)
seed1_val_images = os.listdir(seed1_val_images_folder)

print(f"Seed1: Train images - {len(seed1_train_images)}, Val images - {len(seed1_val_images)}")

cumulative_train_images = set(seed1_train_images)
cumulative_val_images = set(seed1_val_images)
...
# Load the already computed diversity scores and filenames
loaded_results = np.load(diversity_scores_file)
diversity_scores = loaded_results['diversity_scores']
sorted_filenames = loaded_results['sorted_filenames']

# Convert cumulative_train_images to just the filenames, not full paths
cumulative_train_filenames = {os.path.basename(file) for file in previous_seeds_images}

# Filter out filenames already included in seed1 from sorted_filenames
filtered_filenames = [file for file in sorted_filenames if file not in cumulative_train_filenames]

# Select the top 20% of the filtered_filenames
top_20_percent_index = int(len(filtered_filenames) * 0.2)
selected_filenames = filtered_filenames[:top_20_percent_index]

# Load YOLO model
model_path = os.path.join(OUTPUT_DIR, "seed1", 'best_seed1.pt')
model = YOLO(model_path)

# Calculate entropy scores for the selected top 20% images
image_entropy_scores = []
batch_size = 4
for i in range(0, len(selected_filenames), batch_size):
    batch_files = selected_filenames[i:i + batch_size]
    batch_unlabeled_images = [os.path.join(train_images_dir, file) for file in batch_files]

    if not batch_unlabeled_images:  # Skip empty batches
        continue
...
# Select the top half with highest entropy scores
discrepency = 50
top_half_index = int(len(sorted_entropy_scores) * 0.5 + discrepency)
additional_images_train = [img for img, _ in sorted_entropy_scores[:top_half_index]]
...
if not os.path.exists(seed_folder):
    ...
    # Copy images and labels from the seed1
    for file in seed1_train_images:
        shutil.copy(os.path.join(seed1_train_images_folder, file), new_train_images_folder)
        shutil.copy(os.path.join(seed1_train_labels_folder, file.replace('.jpg', '.txt')), new_train_labels_folder)

    for file in seed1_val_images:
        shutil.copy(os.path.join(seed1_val_images_folder, file), new_val_images_folder)
        shutil.copy(os.path.join(seed1_val_labels_folder, file.replace('.jpg', '.txt')), new_val_labels_folder)
    
    print(f"After copying from the seed1 seed: Train images - {count_images_in_folder(new_train_images_folder)}, Val images - {count_images_in_folder(new_val_images_folder)}")
    ...
    final_train_count_seed2 = count_images_in_folder(new_train_images_folder)
    final_val_count_seed2 = count_images_in_folder(new_val_images_folder)
    print(f"Final Seed 4: Train images - {final_train_count_seed2}, Val images - {final_val_count_seed2}")


# Create YAML file for seed2
yaml_dict = {
            "path": ROOT_DIR,
            "train": os.path.join("hybrid/", seed_name, "train/images"),
            "val": os.path.join("hybrid/", seed_name, "val/images"),
            "names": {
                0: "pedestrian",
                1: "people",
                2: "bicycle",
