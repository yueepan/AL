...
tracker = eco2ai.Tracker(
    ...
    experiment_description="hybrid seed3 model",
...
seed2_path = os.path.join(OUTPUT_DIR, "seed2")
seed2_train_images_folder = os.path.join(seed2_path, 'train', 'images')
seed2_val_images_folder = os.path.join(seed2_path, 'val', 'images')
seed2_train_labels_folder = os.path.join(seed2_path, 'train', 'labels')
seed2_val_labels_folder = os.path.join(seed2_path, 'val', 'labels')


seed2_train_images = os.listdir(seed2_train_images_folder)
seed2_val_images = os.listdir(seed2_val_images_folder)

print(f"Seed2: Train images - {len(seed2_train_images)}, Val images - {len(seed2_val_images)}")

cumulative_train_images = set(seed2_train_images)
cumulative_val_images = set(seed2_val_images)
...
# Filter out filenames already included in seed2 from sorted_filenames
...
# Load YOLO model
model_path = os.path.join(OUTPUT_DIR, "seed2", 'best_seed2.pt')
...
if not os.path.exists(seed_folder):
    ...
    # Copy images and labels from the seed2
    for file in seed2_train_images:
        shutil.copy(os.path.join(seed2_train_images_folder, file), new_train_images_folder)
        shutil.copy(os.path.join(seed2_train_labels_folder, file.replace('.jpg', '.txt')), new_train_labels_folder)

    for file in seed2_val_images:
        shutil.copy(os.path.join(seed2_val_images_folder, file), new_val_images_folder)
        shutil.copy(os.path.join(seed2_val_labels_folder, file.replace('.jpg', '.txt')), new_val_labels_folder)
    
    print(f"After copying from the seed2 seed: Train images - {count_images_in_folder(new_train_images_folder)}, Val images - {count_images_in_folder(new_val_images_folder)}")
    ...
    final_train_count_seed3 = count_images_in_folder(new_train_images_folder)
    final_val_count_seed3 = count_images_in_folder(new_val_images_folder)
    print(f"Final Seed 3: Train images - {final_train_count_seed3}, Val images - {final_val_count_seed3}")


# Create YAML file for seed3
yaml_dict = {
            "path": ROOT_DIR,
