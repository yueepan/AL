...
print("Training Images without Labels:")
...
results = model.train(data = os.path.join(ROOT_DIR, "initial seed.yaml"), epochs =1, patience = 20, imgsz= 2560, batch= 4)
...
model = YOLO('best.pt') 
predictions = model.predict(unlabeled_images, save=True, imgsz=256, conf=0.5)
...
print("Copied the initial seed and 2% of labels to the 'seed2'")

os.makedirs(os.path.join(seed_folder, 'labels'), exist_ok=True)
print( f”folder created: {os.path.join(seed_folder, 'labels')}”)

## CREATE EACH COMBINED SEED
...
# Create the seed folders and copy the data from previous seed
seed_folders = []
start_index = 0
...
for i, num_data in enumerate(num_data_per_seed):
    ...
    for j in range(i):
        previous_seed_folder = seed_folders[j]
...
model = YOLO('best.pt')  
results = model.train(data = os.path.join(ROOT_DIR, " seed.yaml"), epochs =100, patience = 20, imgsz= 2560, batch= 4)
